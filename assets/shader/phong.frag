...
void main() {
  ...
  vec3 fragPosition = vec3(modelMatrix * vec4(verPosition, 1.0));
  ...
  if (coefficients.w == 1) {  // Direct light
    ...
    L = normalize(vec3(lightVector) - fragPosition);
    linear = 0.014;
    quadratic = 0.007;
    attenuation = 1.0 / (constant + dist * linear + dist * dist * quadratic);
  }
  ...
  vec3 V = normalize(vec3(viewPosition.x, viewPosition.y, viewPosition.z) - fragPosition);
  ...
  vec3 specularLight = light * ks * pow(max(2 * pow(dot(H, N), 2) - 1, 0.0), 8) * intensity * attenuation;
